...
import CATO from '@/public/partners/cato-networks.svg';

const logos = [
  { src: Huntress, alt: 'logo huntress' },
  { src: SAS, alt: 'logo sas' },
  { src: JUMPCLOUD, alt: 'logo jumpcloud' },
  { src: HTB, alt: 'logo htb' },
  { src: GREMLIN, alt: 'logo gremlin' },
  { src: DATADOG, alt: 'logo datadog' },
  { src: CATO, alt: 'logo cato' },
];
...
const CardPartner = ({ src, alt }: CardPartnerProps) => {
  return (
    <div className="group py-2 px-4 rounded-xl border. border-gray-200. bg-white. bg-opacity-5. hover:bg-opacity-10. flex items-center hover:bg-white">
      <Image
        src={src}
        className="contrast-0 transition group-hover:contrast-1000"
        loading="lazy"
        alt={alt}
        width={120}
        height={100}
